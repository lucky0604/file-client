...
class ExportPanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.file_name_key = wx.StaticText(pnl3, -1, 'Filename',
                                           (60, 120))
            ...
            for file in range(int(file_num)):
                ...
                for j in range(len(df['Filename'])):
                    if list[file] == df['Filename'][j]:
                        df['Status'][j] = 'Sending'
            df.to_excel(self.dir.GetPath() + '/' + 'result.xlsx', sheet_name = 'Sheet1', index = False)
        else:
            df1 = pd.DataFrame({'Filename': list, '': None, 'Status': None})
            for file in range(int(file_num)):
                shutil.move(self.dir.GetPath() + '/' + list[file], self.dir.GetPath() + '/' + export_file_name)
                df1.loc[file]['Status'] = 'Sending'
                df1.to_excel(self.dir.GetPath() + '/' + 'result.xlsx', sheet_name = 'Sheet1', index = True)
...
class MergePanel(wx.Panel):
    ...
    def OnMerge(self, event):
        ...
        for home, dirs, files in os.walk(self.dir2.GetPath()):
            for file in files:
                print(os.path.join(home, file),' ------------- file ---------------')
                if os.path.splitext(file)[1] == '.xml':
                    xml_list.append(file)
        for i in xml_list:
            for j in img_list:
                if os.path.splitext(i)[0] == os.path.splitext(j)[0]:
                    shutil.move(self.dir1.GetPath() + '/' + j, self.dir1.GetPath() + '/result')
                    # shutil.copy()
